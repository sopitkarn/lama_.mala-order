...
export default function OrderPage({ params }) {
  // แก้ไขการ unwrap params ใน Next.js App Router เวอร์ชันใหม่
  ...
  // ฟังก์ชันเพิ่มสินค้าในตะกร้า (จำกัดไม่เกิน 5 ชิ้นต่อ 1 รายการ)
  const handleAddToCart = (item) => {
    setCart((prev) => {
      const currentQty = prev[item.id]?.quantity || 0
      if (currentQty >= 5) return prev
      return {
        ...prev,
        [item.id]: {
          id: item.id,
  ...
  // ฟังก์ชันส่ง Telegram Notification
  const sendTelegramNotification = async (orderItems) => {
    ...
    try {
      ...
      const messageText = `🚨 <b>ออเดอร์ใหม่ - โต๊ะ ${tableNumber}</b>\n\n` +
        `${itemsListHtml}\n\n` +
        `🔗 <a href="${orderUrl}">ดูรายละเอียดออเดอร์</a>`
  ...
  const handleSendOrder = async () => {
    ...
    try {
      ...
      // 2. แสดง Toast และเคลียร์ตะกร้า
      setCart({})
      setSuccessToast(true)
      setTimeout(() => setSuccessToast(false), 3000)

      // 3. ยิง Telegram ในเบื้องหลัง (Background Process)
      sendTelegramNotification(orderItems).catch((err) => {
